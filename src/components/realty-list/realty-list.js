...
import { withDataService, withUpdateFilters } from '../hoc'
...
class RealtyList extends Component {
  ...
  updFilters (filters, realty) {
    ...
    for (const filter of mappedFiltersData) {
      ...
      this.setState({
        filteredRealty: newData,
        count: newData ? newData.length : 0,
        moreBtnView: newData.length <= 5 ? false : true,
        itemsPerPage: 5
      })
    }
  }
  ...
  setItemsCounter () {
    const { filteredRealty, itemsPerPage } = this.state
    const newItemsPerPage = itemsPerPage + 5
    if (filteredRealty.length > newItemsPerPage) {
      this.setState({
  ...
  render () {
    const { realty,
            filteredRealty,
            count,
            moreBtnView } = this.state

    if (!realty) {
      return <Spinner />
    }

    let realtyItems = null
    if (filteredRealty && realty) {
      realtyItems = filteredRealty.slice(0, this.state.itemsPerPage).map((realtyItem) => {
        return (
    ...
    if (count === 0) {
      return (
        <div className='realty-list'>
          <p>К сожалению у нас нет объектов, удовлетворяющих условиям поиска.</p>
        </div>
      )
    }
